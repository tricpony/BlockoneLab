...
class DropDownAnimationController: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let to_vc = (transitionContext.viewController(forKey: .to) as? PopOverViewController),
        let from_vc = transitionContext.viewController(forKey: .from) else {
            return
        }

        let duration = self.transitionDuration(using: transitionContext)
        let presentedFrame = transitionContext.finalFrame(for: (self.direction == .MOVE_IN ? to_vc : from_vc) )
        ...
        transitionContext.containerView.addSubview((self.direction == .MOVE_IN ? to_vc : from_vc).view)
